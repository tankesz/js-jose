...
export class WebCryptographer {
  ...
  createCek (twoOps) {
    ...
    if (twoOps === undefined) {
      ...
      return Jose.crypto.subtle.generateKey(hack.id, true, hack.enc_op.concat(hack.dec_op));
    }
  }
  ...
  resolveEncryptValuePromise (cipherText, tagBytes) {
    const encryptValuePromise = new Promise((resolve, reject) => {
